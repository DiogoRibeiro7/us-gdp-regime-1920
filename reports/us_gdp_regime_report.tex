...
national accounts, we first show that log GDP is highly persistent---standard
unit-root tests do not reject a unit root, though a more powerful GLS-detrended
test does---while its growth rate is unambiguously stationary, so growth---not
the level---is the appropriate object for regime analysis. A dynamic-programming
...
indicate that the log level is highly persistent---though power- and
break-robust tests (DF-GLS and Zivot--Andrews) disagree on whether it is a unit
root---while the growth rate is stationary on every test. Regimes are therefore
estimated on growth. Second, we
...
KPSS test \citep{kpss1992}, whose null is stationarity. Because ADF has low power
and, as \citet{perron1989} shows, unmodelled structural breaks bias it toward
failing to reject a unit root, we add two break- and power-robust checks on the
level: the GLS-detrended DF-GLS test \citep{elliott1996}, and the
\citet{zivotandrews1992} test, whose alternative is trend-stationarity with a
single endogenously dated break.
...
The level's classification is, however, test-dependent
(Table~\ref{tab:breakunitroot}). The more powerful DF-GLS test \emph{rejects} the
unit root ($\LevelDfglsStat$, $p \LevelDfglsP$), whereas the Zivot--Andrews test,
which allows one endogenous break (dated $\ZaBreakYear$), does \emph{not}
($\ZaStat$, $p \ZaP$). This disagreement is exactly the \citet{perron1989}
lesson: whether log GDP reads as a unit root or as a broken trend depends on the
test's power and on how breaks are handled. Nothing in the regime analysis hinges
on resolving it --- growth is stationary on every test, and is what we model ---
but it warns against treating the deterministic trend as structural.
...
\begin{table}[H]
  ...
  \texttt{ct} includes a constant and trend; \texttt{c} a constant only. On these
  two tests the level behaves as difference-stationary and growth as stationary;
  Table~\ref{tab:breakunitroot} qualifies the level result.
  \end{minipage}}
\end{table}

\begin{table}[H]
  \centering
  \caption{Power- and break-robust unit-root tests on the log level}
  \label{tab:breakunitroot}
  \begin{tabular}{lccl}
    \toprule
    Test & Statistic & $p$ & Detail \\
    \midrule
    \input{generated_break_unit_root_table.tex}
  \end{tabular}
  \\[0.4em]
  {\footnotesize\begin{minipage}{0.82\linewidth}
  \textit{Notes.} Null hypothesis: unit root. DF-GLS \citep{elliott1996} is
  GLS-detrended; Zivot--Andrews \citep{zivotandrews1992} allows one endogenously
  dated break in intercept and trend. The two disagree, illustrating the
  \citet{perron1989} power/break caveat.
  \end{minipage}}
\end{table}
...
level is at best highly persistent---its unit-root classification is
test-dependent---while growth is stationary on every test.
...
\bibitem[Elliott et al.(1996)]{elliott1996}
Elliott, G., Rothenberg, T. J., and Stock, J. H.
\newblock Efficient tests for an autoregressive unit root.
\newblock \emph{Econometrica}, 64(4):813--836, 1996.
...
\bibitem[Zivot and Andrews(1992)]{zivotandrews1992}
Zivot, E. and Andrews, D. W. K.
\newblock Further evidence on the Great Crash, the oil-price shock, and the
unit-root hypothesis.
\newblock \emph{Journal of Business \& Economic Statistics}, 10(3):251--270, 1992.
